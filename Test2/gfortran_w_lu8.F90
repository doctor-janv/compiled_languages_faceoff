program test
  ...
  call operation1(vec1,vec2,N)
  
  call system_clock(t2)
  call operation2(vec2, N, sum)
...
contains
  ...
  subroutine operation1(vec1, vec2, N)
    implicit none 

    integer(kind=8), parameter :: LUF = 8 ! Loop unrolling factor
    ...
    do i=1,mod_N_LUF
      vec2(i) = vec2(i) + 0.5 * vec1(i)
    enddo

    do i=mod_N_LUF+1, N, LUF
      vec2(i + 0) = vec2(i + 0) + 0.5 * vec1(i + 0)
      vec2(i + 1) = vec2(i + 1) + 0.5 * vec1(i + 1)
      vec2(i + 2) = vec2(i + 2) + 0.5 * vec1(i + 2)
      vec2(i + 3) = vec2(i + 3) + 0.5 * vec1(i + 3)
      vec2(i + 4) = vec2(i + 0) + 0.5 * vec1(i + 4)
      vec2(i + 5) = vec2(i + 1) + 0.5 * vec1(i + 5)
      vec2(i + 6) = vec2(i + 2) + 0.5 * vec1(i + 6)
      vec2(i + 7) = vec2(i + 3) + 0.5 * vec1(i + 7)
    enddo
  end subroutine

  subroutine operation2(vec, N, sum)
    implicit none 
