...
contains
  ...
  subroutine operation1(vec1, vec2, N, sum)
    ...
    do i=mod_N_LUF+1, N, LUF
      ...
      sum = sum + vec1(i + 2) * vec2(i + 2)
      sum = sum + vec1(i + 3) * vec2(i + 3)
      sum = sum + vec1(i + 4) * vec2(i + 4)
      sum = sum + vec1(i + 5) * vec2(i + 5)
      sum = sum + vec1(i + 6) * vec2(i + 6)
      sum = sum + vec1(i + 7) * vec2(i + 7)
    enddo
  end subroutine
